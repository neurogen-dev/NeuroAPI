...
def get_geoip():
    ...
    if "error" in data.keys():
        ...
        if data["reason"] == "RateLimited":
            return (
                i18n("Ваша IP-зона: неизвестна.")
            )
        else:
            return i18n("Не удалось получить географическое положение IP-адреса. Причина: ") + f"{data['reason']}" + i18n(". Вы все равно можете использовать функцию чата.")
    else:
        ...
        if country == "China":
            ...
            text = i18n("Ваша IP-зона: ") + f"{country}."
        logging.info(text)
        return text
